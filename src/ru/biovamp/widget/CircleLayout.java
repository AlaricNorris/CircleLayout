...
public class CircleLayout extends ViewGroup {
	...
	public CircleLayout(Context context , AttributeSet attrs) {
		...
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs , R.styleable.CircleLayout ,
				0 , 0) ;
		try {
	...
	protected void onLayout(boolean changed , int l , int t , int r , int b) {
		...
		mBounds.set(width / 2 - minDimen / 2 , height / 2 - minDimen / 2 ,
				width / 2 + minDimen / 2 , height / 2 + minDimen / 2) ;
		...
		for(int i = 0 ; i < childs ; i ++ ) {
			...
			final int bottom ;
			if(i == 0 && childs > 1) {
				left = lp.width != LayoutParams.FILL_PARENT ? (x - halfChildWidth * 20 / 10) : 0 ;
				top = lp.height != LayoutParams.FILL_PARENT ? (y - halfChildHeight * 20 / 10) : 0 ;
				right = lp.width != LayoutParams.FILL_PARENT ? (x + halfChildWidth * 20 / 10)
						: width ;
				bottom = lp.height != LayoutParams.FILL_PARENT ? (y + halfChildHeight * 20 / 10)
						: height ;
			}
			else {
				left = lp.width != LayoutParams.FILL_PARENT ? x - halfChildWidth : 0 ;
				top = lp.height != LayoutParams.FILL_PARENT ? y - halfChildHeight : 0 ;
				right = lp.width != LayoutParams.FILL_PARENT ? x + halfChildWidth : width ;
				bottom = lp.height != LayoutParams.FILL_PARENT ? y + halfChildHeight : height ;
			}
			child.layout(left , top , right , bottom) ;
			if(left != child.getLeft() || top != child.getTop() || right != child.getRight()
					|| bottom != child.getBottom() || lp.startAngle != startAngle
	...
	private void drawDividers(Canvas canvas , float halfWidth , float halfHeight , float radius) {
		...
		for(int i = 0 ; i < childs ; i ++ ) {
			...
			canvas.drawLine(halfWidth , halfHeight ,
					radius * (float) Math.cos(Math.toRadians(lp.startAngle)) + halfWidth , radius
							* (float) Math.sin(Math.toRadians(lp.startAngle)) + halfHeight ,
					mDividerPaint) ;
			if(i == childs - 1) {
				canvas.drawLine(halfWidth , halfHeight ,
						radius * (float) Math.cos(Math.toRadians(lp.endAngle)) + halfWidth , radius
								* (float) Math.sin(Math.toRadians(lp.endAngle)) + halfHeight ,
						mDividerPaint) ;
			}
		}
	}
	...
	private void drawInnerCircle(Canvas canvas , float halfWidth , float halfHeight) {
		if(mInnerCircle != null) {
			if( ! (mInnerCircle instanceof ColorDrawable)) {
				mInnerCircle.setBounds((int) halfWidth - mInnerRadius , (int) halfHeight
						- mInnerRadius , (int) halfWidth + mInnerRadius , (int) halfHeight
						+ mInnerRadius) ;
	...
	public void addChild(View inView) {
		addView(inView) ;
	}
}
